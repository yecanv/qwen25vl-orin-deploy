...
新增 [C++17 单图运行时](cpp/README.md)：图像预处理、TensorRT ViT、M-RoPE 和 llama.cpp
集成源码已完成本机编译检查与 CPU 对拍。此版本尚未验证 GPU/Orin 执行，历史板端指标
仍对应原实现；详见 [C++ 验证记录](cpp/VALIDATION.md)。

**目录**

- [1. 项目定位](#1-项目定位)
- [2. 为什么 VLM 部署比纯语言难](#2-为什么-vlm-部署比纯语言难)
- [3. 数据流](#3-数据流)
- [4. 硬件适配矩阵](#4-硬件适配矩阵)
- [5. 快速开始](#5-快速开始)
- [6. 目录](#6-目录)
